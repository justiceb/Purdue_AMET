...
for q = 1:1:length(CD_vec)
    ...
    out.long = sx * dx_2_long + aprs.lng(end);  %3d path from apogee to landing for THIS CD
    out.lat = sy * dy_2_lat + aprs.lat(end);    %3d path from apogee to landing for THIS CD
    ...
    if CD == 1
        descent_path = [[aprs.altitude(1:end); out.alt] ...
                       [aprs.lng(1:end); out.long']     ...
                       [aprs.lat(1:end); out.lat']]
    end

end

%display landing coordinates as a function of CD
landing_path = [[aprs.lat(end) lat_landing]; [aprs.lng(end) long_landing]]'
