...
static void registerMessagesc2_dsm_demo(SFc2_dsm_demoInstanceStruct
  *chartInstance);
...
static void sf_c2_dsm_demo(SFc2_dsm_demoInstanceStruct *chartInstance)
{
  ...
  _SFD_SYMBOL_SCOPE_ADD_EML_IMPORTABLE(&c2_y, 2U, c2_sf_marshallOut,
    c2_sf_marshallIn);
  _SFD_SYMBOL_SCOPE_ADD_EML_IMPORTABLE(chartInstance->c2_A_address, 3U,
  ...
  *c2_b_y = c2_y;
  ...
  _SFD_CHECK_FOR_STATE_INCONSISTENCY(_dsm_demoMachineNumber_,
...
static real_T c2_get_A(SFc2_dsm_demoInstanceStruct *chartInstance, uint32_T c2_b)
{
  ...
  if (chartInstance->c2_A_address == 0) {
    sf_mex_error_message("Invalid access to Data Store Memory data \'A\' (#50) in the initialization routine of the chart.\n");
  }
...
{
  ((real_T *)mxGetPr((plhs[0])))[0] = (real_T)(657849150U);
  ((real_T *)mxGetPr((plhs[0])))[1] = (real_T)(3957442873U);
  ((real_T *)mxGetPr((plhs[0])))[2] = (real_T)(960535639U);
  ((real_T *)mxGetPr((plhs[0])))[3] = (real_T)(1924567029U);
}
...
mxArray *sf_c2_dsm_demo_get_autoinheritance_info(void)
{
  ...
  {
    mxArray *mxChecksum = mxCreateString("RUlbXS6Dlm1ElgsDZD4F6F");
...
mxArray *sf_c2_dsm_demo_third_party_uses_info(void)
{
  ...
  if (!sim_mode_is_rtw_gen(S)) {
    ...
    if (ssIsFirstInitCond(S) && fullDebuggerInitialization==1) {
      /* do this only if simulation is starting */
      {
        ...
        if (chartAlreadyPresent==0) {
          ...
          sf_debug_set_chart_disable_implicit_casting
            (sfGlobalDebugInstanceStruct,_dsm_demoMachineNumber_,
      ...
      sf_debug_reset_current_state_configuration(sfGlobalDebugInstanceStruct,
        _dsm_demoMachineNumber_,chartInstance->chartNumber,
        chartInstance->instanceNumber);
    }
  }
}
...
static void sf_opaque_terminate_c2_dsm_demo(void *chartInstanceVar)
{
  if (chartInstanceVar!=NULL) {
    ...
    if (sim_mode_is_rtw_gen(S) || sim_mode_is_external(S)) {
      ...
      unload_dsm_demo_optimization_info();
    }
...
{
  SFc2_dsm_demoInstanceStruct *chartInstance;
  chartInstance = (SFc2_dsm_demoInstanceStruct *)utMalloc(sizeof
